...
func _ready() -> void:
 ...
 for id in FrontierDiscoveryUtilities.config().buildings:names[id]=FrontierDiscoveryUtilities.config().buildings[id].name
...
func capture_industry() -> void:
 helper.hide()
 var dest:="res://../output/discovery-utilities/"
 ...
 print("DISCOVERY_UTILITIES_RENDER ",suffix);get_tree().quit()
