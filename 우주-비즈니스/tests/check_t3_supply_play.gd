...
func run() -> void:
 ...
 FrontierExpeditionBusiness.transfer(district.inventory,FrontierProductionTier2.batch_cost(FrontierProductionTier2.product(item),3),1)
 ...
 app.open_station("factory",factory.id)
 var production: FrontierProductionPanel=app.business_panel.production_panel
 production.selected_product=item;production.batches.value=3;production.refresh()
 await create_timer(.4).timeout;await capture("production-batches-1280")
 check(production.is_visible_in_tree() and not production.produce.disabled and production.title.text.ends_with("×12"),"live batch selector shows total output and allows production")
 root.size=Vector2i(960,640);root.content_scale_size=root.size;await create_timer(.4).timeout;await capture("production-batches-960")
 check(production.produce.get_global_rect().end.y<=640,"batch production control fits small screen")
 root.size=Vector2i(1280,800);root.content_scale_size=root.size
 production.produce.pressed.emit();app.close_menus()
 check(await until(func():return int(FrontierExpeditionBusiness.site(app.session.authority.world).buildings.get(factory.id,{}).get("product_serial",0))>=3,80),"actual host three-batch P3 production completes")
 ...
 panel.layer=4;panel.refresh();await create_timer(.5).timeout;await capture("supply-plan-1280")
 check(panel.supply_rows.get_child_count()>0,"installed district power and material cards")
 root.size=Vector2i(960,640);root.content_scale_size=root.size;await create_timer(.5).timeout;await capture("supply-plan-960")
 check(panel.info.get_global_rect().end.y<=app.planet_map.get_global_rect().end.y,"supply plan fits small screen")
 app.close_menus()
 app.business_panel.set_context("ship")
 var contracts:=false
 for i in app.business_panel.tabs.get_tab_count():
  if app.business_panel.tabs.get_tab_title(i)=="환경 / 계약":contracts=not app.business_panel.tabs.is_tab_hidden(i)
 check(contracts,"ship contract tab remains reachable")
 var err:=FrontierUniverse.validate_world(app.session.authority.world);check(err.is_empty(),"complete live world saves: "+err)
 await app.session.close_session();app.queue_free();await process_frame;print("T3_SUPPLY_PLAY_FAILURES ",failures);quit(1 if failures else 0)
