...
var folder: String="res://../output/surface-expansion-20260913/after"
...
func run() -> void:
	...
	var rows: Dictionary={}
	var ids: Array=["oxidized","cratered","frozen"]
	var requested:=OS.get_cmdline_user_args()
	for arg in requested:
		if arg=="--after":folder="res://../output/surface-expansion-20260913/after"
	...
	for id in ids:
		...
		camera.look_at_from_position(viewer.position+Vector3(0,.2,2),viewer.position+Vector3(0,-1.5,-4))
		await capture("near-"+id)
		camera.look_at_from_position(viewer.position+Vector3(0,4,10),viewer.position+Vector3(0,-1,-10))
		surface.process_mode=Node.PROCESS_MODE_DISABLED
		var timings: Array=[]
		timings.append(await sample_frames(mat,true))
