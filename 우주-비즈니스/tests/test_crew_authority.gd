...
func run() -> void:
	...
	for i in 5:
		...
		guests.append(guest)
		var accepted:=core.admit(i+2,guest,"",1,FrontierCrewWorld.content_hash())
	...
	check(not core.admit(9,FrontierPlayerProfile.new_character("일곱 번째"),"",1,FrontierCrewWorld.content_hash()).ok,"seventh player rejected")
	check(not core.admit(10,guests[0],tokens[0],1,FrontierCrewWorld.content_hash()).ok,"duplicate character rejected")
	...
	check(not core.admit(12,guests[0],"0".repeat(64),1,FrontierCrewWorld.content_hash()).ok,"nickname or character ID cannot impersonate reconnect")
	var admission:=core.admit(12,guests[0],tokens[0],1,FrontierCrewWorld.content_hash())
	...
	check(core.admit(12,guests[0],tokens[0],1,FrontierCrewWorld.content_hash()).ok and core.acknowledge(12,core.session_id).ok,"reconnect activates the original character")
