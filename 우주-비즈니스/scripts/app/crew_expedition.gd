...
var firearm: FrontierFirearmView
...
func _ready() -> void:
	...
	firearm=FrontierFirearmView.new();add_child(firearm);firearm.configure(self)
...
func _prepare_first_world() -> void:
	...
	await prepare_startup_world(loading)
...
func prepare_startup_world(loading: Node) -> void:
	if not session.active or session.latest.get("phase")!="playing":return
	spaces.sync()
	var local: Dictionary=session.latest.crew.members.get(session.latest.self_id,{})
	if local.get("area","")!="surface" and session.latest.crew.get("landing",{}).is_empty():return
	await loading.checkpoint(86,"행성 지형과 현장 준비")
	var prepared_packet: Dictionary={}
	while session.active:
		_sync_surface_view()
		if surface_world!=null and not is_same(prepared_packet,session.surface):
			surface_world.accept(session.surface);prepared_packet=session.surface
		spaces.sync()
		if session.hosting and surface_world!=null:
			var actor: CharacterBody3D=actors[session.latest.self_id]
			var field:=surface_world.terrain.field
			if field.is_bedrock(actor.position+Vector3.UP):
				actor.position=Vector3(0,field.height(0,0)+2,0);actor.velocity=Vector3.ZERO
				camera.position=actor.position+Vector3.UP*1.72
				session.authority.update_position(1,actor.position);session._publish()
		if surface_world!=null and surface_world.landing_view_ready() and surface_world.incidents.presentation_ready(actors[session.latest.self_id].position):break
		await get_tree().process_frame
	# PhysicsServer must receive the installed shapes before the cover is removed.
...
func _physics_process(delta: float) -> void:
	if preparing_first_snapshot or get_tree().has_meta("startup_loader"):return
	...
	if session.hosting and not session.authority.stopped:
		...
		session.authority.wildlife_obstacle_provider=_wildlife_clear
		session.authority.standing_allowed_provider=_standing_allowed
		...
		for peer in session.authority.peers:
			...
			var wants_sprint: bool=enabled and not member.loadout.get("crouched",false) and input.get("sprinting",false) and direction.length_squared()>0
			...
			if FrontierCrewSurface.landed(local):
				...
				if ground!=null and (ground.field.is_bedrock(actor.position+Vector3.UP) or maxf(absf(actor.position.x),absf(actor.position.z))>float(ground.config.region_half_extent)):
					actor.position=Vector3(0,ground.field.height(0,0)+2,0);actor.velocity=Vector3.ZERO
					motion=FrontierCrewLocomotion.create();session.authority.motions[id]=motion
...
func _predict_local(delta: float,enabled: bool) -> void:
	...
	if not prediction_snapshot.is_empty():
		...
		for frame in prediction_history:
			FrontierCrewLocomotion.step(body,predicted_motion,frame.direction,frame.speed,frame.gravity,frame.jump,frame.delta,frame.enabled,float(frame.get("water",0)),float(frame.get("swim_vertical",0)),float(frame.get("jump_factor",1)),bool(frame.get("jump_held",false)),bool(frame.get("jetpack",false)),surface_world.terrain.field if surface_world!=null else null)
	...
	if on_surface and not firearm.crouched and local_sprint and predicted_motion.grounded and float(member.get("vitals",{}).get("stamina",0))>0 and not member.get("vitals",{}).get("exhausted",false):speed*=float(FrontierCrewVitals.config().sprint_multiplier)
	...
	FrontierCrewLocomotion.step(body,predicted_motion,local_direction,speed,gravity,jump_request,delta,enabled,float(frame.water),float(frame.swim_vertical),float(frame.jump_factor),bool(frame.jump_held),bool(frame.jetpack),surface_world.terrain.field if on_surface and surface_world!=null else null)
...
func _process(delta: float) -> void:
	...
	for id in actors:
		...
		presentation.crouched=members[id].area=="surface" and members[id].get("loadout",{}).get("crouched",false)
...
func interact_flight() -> void:
	...
	# Hold-F investigations own their target; the same press must not also depart or dock.
	if is_instance_valid(flight.freight_view) and not flight.freight_view.selected.is_empty():return
	if is_instance_valid(flight.trace_view) and not flight.trace_view.selected.is_empty():return
	if not flight.atmosphere_target.is_empty():return
	if flight.combat_view!=null and not flight.combat_view.selected_wreck.is_empty():
		session.send_request("space_salvage",{"id":flight.combat_view.selected_wreck});return
	if navigation_ui.interact():return
	if flight.scan_target>=0 and flight.scan_progress>=1.0:navigation_ui.start_route(flight.scan_target)
...
func _update_surface_hud() -> void:
	...
	var text: String="%s  깊이 %.1fm"%[surface_world.body.name,maxf(0,surface_world.terrain.field.height(position.x,position.z)-position.y)]
...
func _crouch_body(actor: CharacterBody3D,low: bool) -> void:
	for child in actor.get_children():
		if child is CollisionShape3D and child.shape is CapsuleShape3D:
			child.shape.height=1.30 if low else 1.86;child.position.y=child.shape.height*.5
func _standing_allowed(id: String) -> bool:
	if not actors.has(id):return false
	var actor: CharacterBody3D=actors[id]
	var query:=PhysicsShapeQueryParameters3D.new();var shape:=SphereShape3D.new();shape.radius=.285
	query.shape=shape;query.transform.origin=actor.position+Vector3.UP*1.57;query.collision_mask=1;query.exclude=[actor.get_rid()]
...
func _wildlife_clear(id: String,encounter_id: String,from: Vector3,to: Vector3,radius: float,height: float) -> bool:
	if not actors.has(id) or spaces==null:return false
	var actor: CharacterBody3D=actors[id]
	var terrain:=spaces.terrain_for(id)
	if terrain==null or not terrain.ready_at(to+Vector3.UP):return false
	var excluded: Array[RID]=[actor.get_rid()]
	# Read the actor's own World3D so independently landed FINCH crews stay isolated.
	var surface: Node=terrain.get_parent()
	if surface is FrontierCrewSurfaceScene and surface.ecology.actors.has(encounter_id):
		for body_node in surface.ecology.actors[encounter_id].get_children():
			if body_node is CollisionObject3D:excluded.append(body_node.get_rid())
	var space_key:=FrontierShuttles.area_key(session.authority.world,id)
	if spaces.spaces.has(space_key) and spaces.spaces[space_key].get("wildlife",{}).has(encounter_id):excluded.append(spaces.spaces[space_key].wildlife[encounter_id].get_rid())
	var space:=actor.get_world_3d().direct_space_state
	if radius<=0:
		var ray:=PhysicsRayQueryParameters3D.create(from,to,1);ray.exclude=excluded
		return space.intersect_ray(ray).is_empty()
	var shape:=CapsuleShape3D.new();shape.radius=minf(radius,height*.5);shape.height=maxf(height,shape.radius*2)
	var query:=PhysicsShapeQueryParameters3D.new();query.shape=shape;query.collision_mask=1;query.exclude=excluded
	query.transform.origin=from+Vector3.UP*(height*.5+.12);query.motion=to-from
	var fractions:=space.cast_motion(query)
	return fractions.is_empty() or fractions[0]>=.99
