...
const CATEGORIES: Array[String]=["all","mineral","gem","product","equipment","specimen"]
func _init() -> void:
	...
	for label in ["전체","광물","보석","부품","장비","표본"]:category.add_item(label)
...
static func kind(resource: String) -> String:
	if FrontierSpecimenItems.is_item(resource):return "specimen"
