...
func configure(owner_app: FrontierCrewExpedition) -> void:
	...
	var heading:=FrontierInterfaceStyle.label(header,"조사 · 개조",25);heading.size_flags_horizontal=Control.SIZE_EXPAND_FILL
...
func _process(_delta: float) -> void:
	if visible:refresh()
func refresh() -> void:
	if app.session.surface.is_empty() or app.session.latest.is_empty():return
