...
const DEFAULTS={"tutorial_mode":0,"preset":1,"scale":1.0,"msaa":1,"fxaa":false,"taa":false,"vsync":true,"fps":60,"view_distance":2400.0,"shadow_distance":120.0,"shadow_size":2048,"shadows":true,"local_shadows":true,"ssao":true,"ssil":false,"ssr":false,"glow":true,"fog":1.0,"lod":3.0,"fov":76.0,"sensitivity":1.0,"invert_y":false,"volume":0.8,"music_volume":0.65,"show_fps":false,"window_mode":0,"resolution":0,"upscaler":0,"sharpness":.2,"shadow_filter":2,"local_shadow_size":1024}
...
var quality_controls: Dictionary={}
var keep_display_button: Button
static var _graphics_groups: Dictionary={}

static func graphics_groups() -> Dictionary:
	if _graphics_groups.is_empty():_graphics_groups=JSON.parse_string(FileAccess.get_file_as_string("res://data/client_graphics.json"))
	return _graphics_groups

func quality_level(group: String) -> int:
	var levels: Array=graphics_groups()[group].levels
	for i in levels.size():
		var matches:=true
		for key in levels[i]:
			if values[key]!=levels[i][key]:matches=false;break
		if matches:return i
	return -1
...
func quality_preset() -> int:
	...
	if level<0:return 3
	for group in ["shadows","effects"]:
		if quality_level(group)!=level:return 3
	return level if quality_level("anti_aliasing")==level+1 else 3

func _merge_quality(group: String,index: int) -> void:
	for key in graphics_groups()[group].levels[index]:
		var value: Variant=graphics_groups()[group].levels[index][key]
		values[key]=int(value) if DEFAULTS[key] is int else value

func set_quality(group: String,index: int) -> void:
...
func _render_scale(page: VBoxContainer) -> void:
	var row:=_row(page,"렌더 해상도")
	var spin:=SpinBox.new();spin.min_value=50;spin.max_value=150;spin.step=5;spin.suffix="%";spin.custom_minimum_size.x=220;row.add_child(spin);controls.scale=spin
	spin.tooltip_text="낮추면 성능이 좋아집니다. 메뉴와 글자의 선명도는 유지됩니다."
	spin.value_changed.connect(func(value: float):set_option("scale",value/100.0))
...
func _sync() -> void:
	rebuilding=true
	values.preset=quality_preset()
	for group in quality_controls:
		var index:=quality_level(group)
		quality_controls[group].select(index if index>=0 else graphics_groups()[group].levels.size())
	for key in controls:
		...
		else:control.value=values[key]*100.0 if key=="scale" else values[key]
...
func _build() -> void:
	...
	var theme:=Theme.new();var font:=FontVariation.new();font.base_font=load("res://assets/fonts/NotoSansKR.ttf");font.variation_opentype={TextServerManager.get_primary_interface().name_to_tag("wght"):550.0};theme.default_font=font;theme.default_font_size=16
	...
	overlay.theme=theme
	var heading:=Label.new();heading.text="설정";heading.add_theme_font_size_override("font_size",28);column.add_child(heading)
	var subtitle:=Label.new();subtitle.text="변경한 설정은 자동으로 저장됩니다.";column.add_child(subtitle)
	tabs=TabContainer.new();tabs.size_flags_vertical=Control.SIZE_EXPAND_FILL;column.add_child(tabs)
	var graphics:=_page("그래픽")
	_choice(graphics,"전체 품질","preset",["낮음","보통","높음","사용자 설정"],[0,1,2,3])
	...
	_choice(display,"창 해상도","resolution",["1280 × 800","1280 × 720","1600 × 900","1920 × 1080","2560 × 1440"],[0,1,2,3,4])
	...
	for entry in [["닫기 · Esc / F10",close],["화면 변경 유지",func():display_previous.clear();notice.text="화면 설정을 저장했습니다." if save_settings() else "설정 저장 실패"],["기본값 복원",func():
		...
		if entry[0]=="화면 변경 유지":keep_display_button=button;button.hide()
