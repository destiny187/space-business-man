...
func configure(owner_app: FrontierCrewExpedition) -> void:
	...
	depart.pressed.connect(func():app.travel_action("tutorial_depart"));depart.hide()
...
func update_snapshot(value: Dictionary) -> void:
	...
	depart.visible=solar and nav.mode=="idle" and not letter.visible
	depart.disabled=value.self_id!=value.crew.pilot_id
	depart.text="출격하기  [G]" if not depart.disabled else "조종사의 출격을 기다리는 중"
	if float(nav.get("energy",100))<30:depart.disabled=true;depart.text="추진 에너지 충전 중"
