...
func _layout() -> void:
	...
	hotbar.position=Vector2((viewport_size.x-384)/2,viewport_size.y-88)
	_filter_items()
...
func _process(delta: float) -> void:
	...
	for storage_index in 2:
		...
		for stack in displayed:
			...
			var tile:=FrontierItemTile.new();tile.picture=FrontierResourceIcons.texture(id);tile.amount=str(int(stack.amount));tile.caption=FrontierCatalog.entry("resources",id).name;FrontierItemBrowser.tag(tile,tile.caption,FrontierItemBrowser.kind(id),int(source[id]));tile.tooltip_text=FrontierCatalog.entry("resources",id).name+(" · 최대 %d개"%int(FrontierItemInventory.config().resource_stack) if storage_index==0 else " · 공동 재고 합계")
...
func _refresh_details() -> void:
	...
	if tabs.current_tab==1:
		for id in def.cost:
			var cost:=VBoxContainer.new();materials.add_child(cost);cost.add_child(FrontierResourceIcons.view(id,30));var have:=int(bag.get(id,0));FrontierInterfaceStyle.label(cost,("부족 · %d 필요"%int(def.cost[id])) if have<=0 else "%d/%d"%[have,int(def.cost[id])],12,FrontierInterfaceStyle.ACCENT if have>=int(def.cost[id]) else FrontierInterfaceStyle.WARNING)
		...
		if upgrade_action.visible:
			for resource in next.get("cost",{}):
				var column:=VBoxContainer.new();materials.add_child(column);column.add_child(FrontierResourceIcons.view(resource,28));FrontierInterfaceStyle.label(column,("부족 · %d 필요"%int(next.cost[resource])) if int(bag.get(resource,0))<=0 else "%d/%d"%[int(bag.get(resource,0)),int(next.cost[resource])],12)
