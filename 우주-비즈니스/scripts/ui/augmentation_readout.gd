...
func configure(owner_app: FrontierCrewExpedition) -> void:
	...
	var visit:=Button.new();visit.text="강화";visit.tooltip_text="신체 증강 장치";header.add_child(visit);visit.pressed.connect(func():app.stations.navigate("augmentation"))
	identity=FrontierInterfaceStyle.label(header,"",22);identity.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART;identity.size_flags_horizontal=Control.SIZE_EXPAND_FILL
