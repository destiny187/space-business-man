...
func configure(owner_app: FrontierCrewExpedition,is_factory: bool=false) -> void:
	app=owner_app;factory=is_factory;name="로버 제작" if factory else "운송 숙련"
...
func _process(_delta: float) -> void:
	...
	if factory:
		...
		var access: String=app.stations.work_reason("augmentation")
		if not access.is_empty():reason=access
		elif level>=2:reason="운송 개조 완료  적재/하역 8초 → 5초"
		elif FrontierCrewWorld.vector(member.position).distance_to(FrontierCrewWorld.vector(FrontierCrewSurface.config().ship_position))>float(FrontierCrewSurface.config().boarding_distance):reason="착륙선 증강 장치에 접근하세요"
