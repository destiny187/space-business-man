...
func _draw() -> void:
	...
	var bg: Color=FrontierInterfaceStyle.HOVER if is_hovered() else FrontierInterfaceStyle.PANEL
	...
	if rarity_color.a>0:draw_rect(Rect2(7,size.y-3,size.x-14,2),rarity_color)
	var font:=get_theme_default_font()
	var picture_area:=Rect2(7,5,size.x-14,size.y-28)
	if not rarity_label.is_empty():picture_area=Rect2(7,23,size.x-14,size.y-48)
	...
	if picture!=null:
		...
		if placeholder_icon:ratio=minf(ratio,28.0/maxf(picture.get_width(),picture.get_height()))
		var dims:=picture.get_size()*ratio
		draw_texture_rect(picture,Rect2(picture_area.position+(picture_area.size-dims)*.5,dims),false,FrontierInterfaceStyle.MUTED if placeholder_icon else Color(1,1,1,.45 if unavailable else 1))
	else:
