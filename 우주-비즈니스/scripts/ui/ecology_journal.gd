...
func configure(owner_app: FrontierPlanetExploration) -> void:
	...
	discoveries=OptionButton.new();discoveries.custom_minimum_size.y=38;discoveries.fit_to_longest_item=false;column.add_child(discoveries)
	...
	var analyze_button:=Button.new();analyze_button.text="우주선에서 기초 분석 · 창고 광물 3";analyze_button.pressed.connect(func():app.ecology_action("analyze",selected_form());refresh());column.add_child(analyze_button)
	...
	column.add_child(plot_button)
	var cargo_title:=Label.new();cargo_title.text="격리 표본 · 우주선과 함께 운송";column.add_child(cargo_title)
	cargo=OptionButton.new();cargo.fit_to_longest_item=false;cargo.custom_minimum_size.y=38;column.add_child(cargo)
	...
	var refill:=Button.new();refill.text="지원 팩 보충 · 창고 광물 3";refill.pressed.connect(func():app.ecology_action("resupply");refresh());column.add_child(refill)
	lab=Label.new();lab.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART;lab.add_theme_color_override("font_color",Color("a6c4be"));column.add_child(lab)
...
func refresh() -> void:
	...
	form_ids.sort()
	for id in form_ids:discoveries.add_item(FrontierEcologyCatalog.form(id).name)
	...
	for id in sample_ids:
		...
		cargo.add_item("%s · %s"%[FrontierEcologyCatalog.form(row.form_id).name,FrontierUniverse.body_from_id(app.state.manifest,row.source_body).name])
