...
func _ready() -> void:
	super._ready()
	configure("항해 기록","선택 항로 설정","탐험과 생산 거점","ship",true)
	secondary.text="닫기";primary.disabled=true
	confirmed.connect(func():
		if not entries.get_selected_items().is_empty():selected.emit(int(entries.get_item_metadata(entries.get_selected_items()[0]))))
	...
	entries=ItemList.new();entries.add_theme_stylebox_override("panel",FrontierInterfaceStyle.box());entries.size_flags_vertical=Control.SIZE_EXPAND_FILL;entries.custom_minimum_size.y=220;entries.auto_height=false;column.add_child(entries)
	entries.item_activated.connect(func(i):selected.emit(int(entries.get_item_metadata(i)));hide())
	entries.item_selected.connect(func(_index):primary.disabled=false)
	var actions:=HBoxContainer.new();column.add_child(actions);column.move_child(actions,entries.get_index())
	button(actions,"이전",func():page=maxi(0,page-1);refresh())
	button(actions,"다음",func():if (page+1)*50<pages.size():page+=1;refresh())
	button(footer,"Earth 귀환",func():
		if visible:hide();selected.emit(int(journal.manifest.settings.get("starting_ordinal",2))))
...
func refresh() -> void:
	...
	pages=journal.ordinals(filter.selected,"");entries.clear();primary.disabled=true
