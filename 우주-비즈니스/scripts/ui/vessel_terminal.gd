...
func configure(owner_panel: FrontierBusinessPanel) -> void:
	...
	for row in [["shuttles","소형선 호출","기체 선택  위치 지정","ship_module"],["lotus","Lotus 보급","기초 물자  현장 투하","ice"],["cargo","화물창","배낭 ↔ 선박","stone"],["inventory","내 아이템","장비  번호 슬롯","reinforced_frame"],["research","연구","설계도  생태 분석","crystal"],["shipyard","정비","선체  모듈","control_circuit"],["launch","탑승  이륙","승무원 탑승 후 출항","ship_module"],["rejoin","원정선 합류","화물은 먼저 직접 하역","ship_module"]]:
...
func update_snapshot(value: Dictionary) -> void:
	...
	for id in ["research","shipyard","shuttles"]:cards[id].visible=not personal
