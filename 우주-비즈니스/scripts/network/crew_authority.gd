...
func start(source: Dictionary,profile: Dictionary,persist: Callable) -> bool:
	...
	FrontierFacilityResearch.migrate(world)
...
func request(peer: int,envelope: Variant,from_queue: bool=false) -> Dictionary:
	...
	if (not from_queue and not queued_requests.is_empty()) or (autonomous_pending() and not resolve_autonomous()):
		...
		# Keep a short, bounded trigger request while the durable state commits.
		# The queue still validates ownership, menu state and cadence at execution.
		if envelope.kind=="surface_fire" and queued_requests.any(func(q):return q.actor==actor and q.envelope.kind=="surface_fire"):
			return {"ok":false,"code":"trigger_buffer_full","firearm_action":envelope.kind}
		if queued_requests.size()>=32:return failure("앞선 작업을 처리 중입니다. 잠시 후 다시 시도하세요.")
		queued_requests.append({"peer":peer,"actor":actor,"envelope":envelope.duplicate(true),"queued_at":now})
		return {"pending":true}
	...
	if FrontierCrewSurface.landed(world) and not FrontierCrewSurface.landed(draft):FrontierShuttles.stow_docked(draft)
...
func firearm_command(peer: int,envelope: Dictionary) -> Dictionary:
	...
	if result.get("ok",false):
		...
		gun_dirty=true
	return result
func input(peer: int,sequence: int,direction: Variant,aim_value: Variant=[],scanning: bool=false,sprinting: bool=false,flight_controls: Array=[0.0,0.0,0.0],jump_request: int=0,controls_enabled: bool=true,vehicle_controls: Array=[],weather_ready: bool=false,jump_held: bool=false) -> bool:
