...
func send_input(direction: Vector2,aim: Vector3=Vector3.FORWARD,scanning: bool=false,sprinting: bool=false,flight_controls: Array=[0.0,0.0,0.0],jump_request: int=0,controls_enabled: bool=true,vehicle_controls: Array=[],weather_ready: bool=false,jump_held: bool=false) -> void:
	if not active:return
	movement_sequence+=1
	if hosting:authority.input(1,movement_sequence,[direction.x,direction.y],[aim.x,aim.y,aim.z],scanning,sprinting,flight_controls,jump_request,controls_enabled,vehicle_controls,weather_ready,jump_held)
	else:_movement.rpc_id(1,session_id,movement_sequence,[direction.x,direction.y],[aim.x,aim.y,aim.z],scanning,sprinting,flight_controls,jump_request,controls_enabled,vehicle_controls,weather_ready,jump_held)
@rpc("any_peer","call_remote","unreliable_ordered",1)
func _movement(epoch: String,sequence: int,direction: Array,aim: Array=[],scanning: bool=false,sprinting: bool=false,flight_controls: Array=[0.0,0.0,0.0],jump_request: int=0,controls_enabled: bool=true,vehicle_controls: Array=[],weather_ready: bool=false,jump_held: bool=false) -> void:
	if hosting and epoch==session_id:authority.input(multiplayer.get_remote_sender_id(),sequence,direction,aim,scanning,sprinting,flight_controls,jump_request,controls_enabled,vehicle_controls,weather_ready,jump_held)
