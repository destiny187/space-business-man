...
static func step(authority: FrontierCrewAuthority,peer: int,local: Dictionary,delta: float) -> bool:
	...
	var cfg:=FrontierFreightSalvage.rules(local.manifest)
	progress+=delta/float([cfg.identify_seconds,cfg.recover_seconds,cfg.handover_seconds][stage])
	...
	if stage==2:
		...
		draft.business.credits+=int(cfg.payment)
	...
	authority.world=draft;scan.stage=stage+1;scan.progress=0.0;scan.known=stage==2;authority.scans[peer]=scan
	return true
