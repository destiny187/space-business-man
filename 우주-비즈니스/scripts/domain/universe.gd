...
static func _make_body(m: Dictionary, ordinal: int, corporate: bool=true) -> Dictionary:
	...
	if corporate and int(s.ordinal)>0 and FrontierCorporateSites.enabled(m):
		var site:=FrontierCorporateSites.body_info(m,ordinal)
		if not site.is_empty():
			result.corporate_activity=site.duplicate(true)
			if site.managed:result.management={"operator":"space_y","state":"restored","access":"orbital_only","pressure":.9,"cloud":.42,"city_strength":.38}
	return result
...
static func landable(body_value: Dictionary) -> bool:
	return body_value.get("origin","")!="solar_reference" and body_value.get("landable",true) and body_value.get("management",{}).get("access","")!="orbital_only"
...
static func validate_world(value: Variant) -> String:
	...
	if value.has("crew"):
		...
		for id in FrontierFreightSalvage.records(value):
...
static func _validate_terrain(value: Dictionary) -> String:
	...
	if value.has("terrain_settings"):
		...
		for point in value.get("surface_positions",{}).values():
			if absf(point[0])>float(cfg.region_half_extent) or absf(point[2])>float(cfg.region_half_extent) or point[1]<minf(-512.0,float(cfg.minimum_depth))+2 or point[1]>float(cfg.maximum_height)+span-2:return "저장된 지표 위치가 탐사 영역 밖입니다."
