...
static func system_of(id: String) -> int:
	var suffix:=id.trim_prefix("freight-v1:");var system:=int(suffix)
	return system if suffix.is_valid_int() and system>=0 and system<125000 and address(system)==id else -1
...
static func valid(rows: Variant,crew: Dictionary) -> bool:
	if not rows is Dictionary or rows.size()>125000:return false
	var occupied: Dictionary={}
	for id in rows:
		if not id is String or system_of(id)<0:return false
		var row: Variant=rows[id]
		if not row is Dictionary or not FrontierExpeditionBusiness.integer(row.get("stage"),1,3) or not row.get("carrier") is String or not FrontierUniverse._finite(row.get("at"),0,9007199254740000):return false
		if int(row.stage)==1:
...
static func snapshot(rows: Dictionary,system: int) -> Dictionary:
	...
	for id in rows:
		if int(rows[id].stage)==2 or id==address(system):result[id]=rows[id].duplicate()
	return result
...
static func target(m: Dictionary,nav: Dictionary,aim: Vector3,rows: Dictionary,vessel: String) -> Dictionary:
	...
	var row:=definition(m,address(int(nav.system)),float(nav.get("orbit_time",0)))
	if row.is_empty():return {}
	var record: Dictionary=rows.get(row.id,{"stage":0,"carrier":""})
	row.stage=int(record.stage);row.carrier=record.carrier
	if row.stage==2 and row.carrier!=vessel:return {}
	row.position=row.receiver if row.stage>=2 else row.position
	var point:=FrontierCrewWorld.vector(row.position);var origin:=FrontierCrewWorld.vector(nav.position);var offset:=point-origin
	if offset.length()>float(rules(m).signal_distance) or offset.normalized().dot(aim.normalized())<.985 or FrontierCorporateTraces.occluded(m,int(nav.system),float(nav.get("orbit_time",0)),origin,point):return {}
...
static func reason(m: Dictionary,nav: Dictionary,target: Dictionary,rows: Dictionary,vessel: String,pilot: bool) -> String:
	...
	if stage==3:return "인계 대금 지급 완료 · J"
...
static func drift(world: Dictionary,old_time: float) -> bool:
	...
	var drift_vector:=FrontierCrewWorld.vector(row.receiver)-FrontierCrewWorld.vector(previous.receiver)
...
static func obstacles(m: Dictionary,system: int,t: float,rows: Dictionary={}) -> Array:
	var row:=definition(m,address(system),t)
	if row.is_empty():return []
	var result: Array=[{"point":FrontierCrewWorld.vector(row.receiver),"radius":85.0}]
	if int(rows.get(row.id,{}).get("stage",0))<2:result.append({"point":FrontierCrewWorld.vector(row.position),"radius":50.0})
	return result
