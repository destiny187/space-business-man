...
static func guard(world: Dictionary,actor: String,kind: String,args: Dictionary) -> String:
	...
	if aboard(world,actor):
		...
		if kind in ["business_robot_deploy","business_robot_recover","surface_collect","surface_analyze","surface_introduce","surface_restore","surface_resupply"]:return "FINCH는 자원 운송선입니다. 로봇 격납고·표본 연구는 공동 원정선을 이용하세요."
