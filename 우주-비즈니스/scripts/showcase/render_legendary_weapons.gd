...
func run() -> void:
	...
	for kind in ["legend_ember","legend_shatter","legend_arc","legend_pierce"]:
		...
		for i in scene.samples.size():
			...
			var image:=root.get_texture().get_image();image.save_png("res://../docs/production/media/weapon-loot/"+kind+"-godot.png")
