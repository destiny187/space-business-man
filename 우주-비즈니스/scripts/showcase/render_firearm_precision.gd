...
func run() -> void:
	...
	for kind in ["gun_laser","ammo_energy","firearm_hand_right","firearm_hand_left"]:
		...
		for i in scene.samples.size():
			...
			var image:=root.get_texture().get_image();image.save_png("res://../docs/production/media/firearm-precision/"+kind+"-godot.png")
