...
var records: Array=[]
func run() -> void:
	folder=ProjectSettings.globalize_path("res://../output/creature-remodel/combat");DirAccess.make_dir_recursive_absolute(folder)
	...
	var manifest: Dictionary=JSON.parse_string(FileAccess.get_file_as_string("res://data/creature_remodel_combat.json"))
	for form in manifest.forms:
		for mode in ["hit","miss","blocked"]:await exercise(stage,form,mode)
...
func exercise(stage: Node3D,form: Dictionary,outcome: String) -> void:
	...
	var target:=home+Vector3(0,0,4.5);target.y=field.height(target.x,target.z)+.1;member.position=FrontierExplorationIncidents.array(target)
	...
	var obstacle:=func(_a,_b,_from,to,_radius,_height):return outcome!="blocked" or Vector3(to).z<home.z+2.8
	...
	for i in ceili(duration*30):
		...
		root_error=maxf(root_error,actor.global_position.distance_to(at));max_error=maxf(max_error,actor.ground_motion.grounded_error);clips[actor.ground_motion.wanted_clip]=true
	...
	check(damage_events==1 if outcome=="hit" else damage_events==0,"host contact outcome "+outcome)
	...
	if outcome=="hit":check(clips.has("charge_loop") if info.behavior=="charge" else clips.has("leap_air") and clips.has("leap_land"),"host-compatible clips")
