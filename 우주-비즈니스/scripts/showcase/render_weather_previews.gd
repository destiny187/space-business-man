extends SceneTree
## Refresh only the two weather facility cards with the production INK renderer.
...
func run() -> void:
	...
	for kind in ["field_canopy","grounding_mast"]:
		for i in scene.samples.size():
			...
			var image:=root.get_texture().get_image();image.save_png("res://../docs/production/media/weather/"+kind+"-godot.png")
