...
func run() -> void:
	...
	for kind in ["ammo_light","ammo_heavy","ammo_shell","ammo_sniper","ammo_plasma","firearm_hand_right","firearm_hand_left"]:
		if not OS.get_cmdline_user_args().is_empty() and not kind in OS.get_cmdline_user_args():continue
		for i in scene.samples.size():
			...
			var image:=root.get_texture().get_image();image.save_png("res://../docs/production/media/firearm-upgrade/"+kind+"-godot.png")
			...
			if kind.begins_with("ammo_"):image.save_png("res://assets/ui/resources/"+kind+".png")
