...
func _process(delta: float) -> void:
	...
	for row in pending:
		if resource_requests.size()>=2:break
		...
		resource_requests[row.id]={"paths":paths}
...
func refresh() -> void:
	...
	for candidate in FrontierEcologyPlacement.candidates(body,record,viewer.position):
		if selected.size()>=int(FrontierEcologyCatalog.config().max_actors):break
		...
		if point.distance_to(viewer.position)>float(FrontierEcologyCatalog.config().active_radius):continue
		...
		if status_value=="active":active_count+=1
	...
	for id in resource_requests.keys():
		...
		var completed:=true
		for path in resource_requests[id].paths:
			var status_value: int=ResourceLoader.load_threaded_get_status(path)
