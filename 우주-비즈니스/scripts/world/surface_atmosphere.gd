...
func update_cycles() -> void:
	...
	sun.light_energy=float(cycles.sun_energy)*smoothstep(-.02,.10,direction.y)*(1.0-weather_clouds*.6)
...
func paint_cycles() -> void:
	material.set_shader_parameter("cloud_amount",maxf(float(current.cloud_amount),weather_clouds))
	material.set_shader_parameter("storm_cover",weather_clouds)
	if cycles.is_empty():return
	...
	material.set_shader_parameter("cloud_color",Color("192838").lerp(current.cloud_color,daylight).lerp(Color("986a66"),dusk*.35).lerp(Color("667d89"),weather_clouds*.7))
