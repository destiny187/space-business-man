...
func configure(system: Dictionary,radius: float) -> void:
	...
	for key in ["mode","surface_speed","cell_scale"]:surface.set_shader_parameter(key,profile[key])
	...
	var sunlight:=OmniLight3D.new();sunlight.light_color=Color(profile.tint);sunlight.light_energy=2.4;sunlight.omni_range=90000;sunlight.omni_attenuation=.25;add_child(sunlight)
