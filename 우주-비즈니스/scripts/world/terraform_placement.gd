...
func _process(dt: float) -> void:
 ...
 visible=FrontierFreeTerraform.active(site) and kind in site.get("free_terraform",{}).get("rules",{}).get("radius",{})
 if not visible:return
 var p:=Vector2(global_position.x,global_position.z);var radius:=FrontierFreeTerraform.radius(site,{"type":kind})
 var inside:=FrontierFreeTerraform.in_pollution(site,p);var tint:=Color(.4,.9,.72,.8)
