"""Import two authorized ElevenLabs web generations; keep raw WAV and mono game mix."""
from pathlib import Path
import array,hashlib,json,math,shutil,wave
from datetime import datetime,timezone
ROOT=Path(__file__).resolve().parents[1]
JOBS=[
('amb_discovery_resonance','Wind_passing_through',True,'Wind passing through hollow basalt pillars produces gentle low flute-like resonances with airy overtones, an unusual natural geological sound. Steady close soft wind resonance for a science fiction exploration game. No music, voice, melody, percussion or harsh whistle.'),
('sfx_discovery_excavate','A_compact_geology_to',False,'A compact geology tool breaks a thin brittle mineral crust: three crisp stone cracks, small crystal fragments falling, soft dust settling, one short close dry excavation action. Science fiction exploration game foley. No explosion, music, voices, chime or distant ambience.')]
...
(ROOT/'audio/manifests/exploration-discoveries.json').write_text(json.dumps({'date':'2026-09-09','jobs':records},ensure_ascii=False,indent=2)+'\n')
