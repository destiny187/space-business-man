"""Import three authorized ElevenLabs encounter cues; preserve original WAVs."""
from pathlib import Path
import array,hashlib,json,math,shutil,wave
from datetime import datetime,timezone
ROOT=Path(__file__).resolve().parents[1]
JOBS=[('sfx_incident_quake', 'A_subterranean_earth', False, 'A subterranean earthquake cracks open a stone passage. Deep low rumble builds, sharp rock fractures and falling gravel, a compact pressure burst with debris, then decays. Four seconds, isolated game sound effect, no music or voices.'), ('sfx_incident_robot_wake', 'A_dormant_heavy_indu', False, 'A dormant heavy industrial combat robot powers on and rises: rusty articulated joints unlock, hydraulic servos lift the torso, cooling fans accelerate and a brief electronic warning chirp ends the sequence. Four seconds, close dry science fiction game foley, no music, speech or weapon fire.'), ('sfx_incident_beacon', 'One_short_soft_elec', False, 'One short soft electronic rescue beacon ping, two clear rounded notes and a faint radio static tick, an old spacecraft distress locator received inside an exploration helmet. A single compact half-second signal followed by silence. No music, speech, rhythm or alarm siren.')]
...
(ROOT/'audio/manifests/exploration-incidents.json').write_text(json.dumps({'date':'2026-09-09','jobs':records},ensure_ascii=False,indent=2)+'\n')
