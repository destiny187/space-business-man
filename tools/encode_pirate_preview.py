...
parser=argparse.ArgumentParser();parser.add_argument('--capture',choices=['pirate-quality','pirate-explosion','pirate-missiles'],default='pirate-quality');args=parser.parse_args()
...
(out.parent/'movie-provenance.json').write_text(json.dumps({**meta,'start_seconds':start,'duration_seconds':duration,'game_scene':'res://scenes/app/crew_expedition.tscn','capture_script':{'pirate-quality':'res://tests/render_pirate_motion.gd','pirate-explosion':'res://tests/render_pirate_explosion.gd','pirate-missiles':'res://tests/render_pirate_missiles.gd'}[args.capture],'render':'Godot 4.7.2 Forward+ / Metal; Movie Maker fixed 60 FPS','note':'Scripted pilot inputs through the actual host; offline rendering, not a real-time FPS benchmark; original mixed audio retained'},ensure_ascii=False,indent=2)+'\n')
