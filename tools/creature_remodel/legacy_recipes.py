...
def recipes():
    ...
    for file in ['forms','xenofauna_forms']:
        for f in json.loads((ROOT/f'우주-비즈니스/data/bestiary/{file}.json').read_text())['forms']:
            ...
            morphology=dict(width=p.get('width',1+.035*(mode%5)),height=p.get('height',1+.025*(mode//5)),mode=int(p.get('branch_mode',mode)),radial_count=p.get('radial_count',3+mode%5),limb_count=p.get('limb_count',4+2*(mode%3)),eye_count=f.get('eye_count',2))
            ...
            palette=f['palette'];rows.append(dict(id=f['id'],source_id=f['id'],name=f['name'],family=f['family'],kind=f['family'],construction=f['family'],organ_system=ORGANS[mode%7],environment=f['environment'],habitat=f.get('habitat_note',''),locomotion_medium=f.get('locomotion_medium','ground' if ground else ('water' if f['family'] in ['ray','swimmer'] else 'air')),anatomy_note=f.get('anatomy_note','Original family rebuilt with articulated animal anatomy'),morphology=morphology,attack=f.get('attack','none'),palette=[palette[0],palette[1],'34494e',palette[2]],host_pattern=pattern,host_motion=combat['patterns'].get(pattern,{}).get('behavior',''),production_batch='r06',source_renderer='BLENDER_EEVEE'))
