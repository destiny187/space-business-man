"""Author 36 original geological scatter meshes in Blender, export and render."""
...
for idx,key in enumerate(ids):
    ...
    t=rules['archetypes'][key];rng=random.Random(3701+idx*971)
...
(SRC/'manifest.json').write_text(json.dumps({'version':1,'blender':bpy.app.version_string,'assets':records},ensure_ascii=False,indent=2)+'\n')
...
scene.render.image_settings.file_format='PNG';scene.render.filepath=str(PIC/'blender-catalog.png')
bpy.ops.wm.save_as_mainfile(filepath=str(SRC/'catalog.blend'));bpy.ops.render.render(write_still=True)
