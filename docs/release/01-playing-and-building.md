...
시작 화면에는 **혼자 게임 시작/이어하기**, **함께 플레이 · 최대 6명**, 종료만 제공한다. 이전 구매형·단독 실증 시작 항목은 제거했다. 혼자는 연결 설정과 별도 준비 버튼 없이 출발/착륙하며, 착륙하면 항해 패널이 닫힌다. B 사업·건설, J 생태 연구, Tab 항해, Esc 닫기를 사용한다. 혼자 세계는 `solo_world.json`에 저장하고 기존 저장은 보존한다. 아래 기존 1.2 조작/첫 사업 설명은 과거 모드 기록이며 현재 진입 경로가 아니다. [수정·검증 기록](../production/17-solo-entry-and-surface-ui.md).
