...
홈 우측 하단의 `버전 YYYY.MM.DD+커밋번호`로 실행 중인 소스를 구분한다. 소스 실행은 시작할 때 현재 Git 커밋을 한 번 조회하므로 새 변경을 커밋해 push/pull하면 다음 실행에 자동 반영된다. 같은 커밋의 재전송은 같은 버전이다. 미커밋 수정이나 ignore 대상이 아닌 새 파일이 있으면 `(개발 중)`을 붙이며 이를 배포 확정판으로 표시하지 않는다.

macOS/Windows 빌드 스크립트는 내보내기 전에 `tools/stamp_build_version.py`로 `data/build_version.json`을 생성해 PCK에 넣는다. 직접 Godot 내보내기를 사용하는 경우에도 먼저 이 스크립트를 실행한다. 이 파일은 빌드 산출물이며 Git 추적 대상이 아니다. Git 정보와 내장 버전이 모두 없으면 버전 정보 없음으로 표시한다.

이미 받은 앱/ZIP은 push만으로 바뀌지 않는다. 새 코드와 버전이 담긴 배포 파일은 빌드 스크립트로 다시 생성해야 한다. 홈 표시와 기존 OS 패키지의 `1.2.0` 메타데이터는 구분한다. [구현·확인 경계](../production/192-freight-hold-and-home-version.md).

## 2026-09-13 Windows 현재 작업본 패키지

현재 작업 폴더의 미커밋 변경을 포함해 Windows x64 ZIP을 다시 생성했다. 내부 버전은 1.2.0이며 다운로드 전달명은 `Locus-1.2.0-20260913-1640-Windows-x64.zip`, 크기는 13,942,741,923바이트다. SHA-256은 `e12c8d63401290e2f769777075ee0b14505d604899c48066df8908409b11b14e`이며 다운로드 복사본도 일치했다.

ZIP CRC·구성 파일 해시 9개·PE x64와 PCK 리소스 56,048개를 검사했다. 현재 모델 16,349개와 음원 109개의 포함 여부를 확인했다. Windows 실기 실행과 이번 패키지의 게임 화면·음향 검수는 수행하지 않았다. 결과는 `test-results/windows-release.json`에 남긴다.

대용량 파일 전체를 메모리에 올리던 패키징·검사 해시를 1MiB 단위로 읽도록 바꿨다. PCK도 디렉터리와 리소스를 나누어 읽으며, CRC는 압축 해제 중 검사해 중복 압축 해제를 없앴다. 배포 설정에서 제외한 제작용 JSON은 필수 포함 검사에서도 제외한다. ZIP은 DEFLATE 6을 사용하고 `build-info.json`에 미커밋 변경 여부를 기록한다. 게임 행동·저장·네트워크·화면 갱신 경로는 변경하지 않았다.
...
## 새 은하 시작 경로 (2026-09-06)

메인 화면의 ‘새 은하에서 혼자 시작’은 기존 기록을 보존하고 새 무작위 은하의 지구 궤도로 진입한다. 기본 혼자 버튼은 최근 세계를 연다. ‘함께 플레이’에서 세계 열기/새 은하로 방 만들기 또는 주소로 참가를 선택하고, 별도 대기실에서 참가자 준비 후 호스트가 게임 시작을 누른다. 진행 중 방에 참가하면 현재 진행에 합류한다.

항해 패널에서 궤도도의 천체·이름 목록 또는 은하 지도의 대표 항성계를 선택한다. 행성 주소 1~1000000도 사용 가능하다. 가스/얼음 거대행성은 궤도 접근만 가능하며 착륙 버튼이 비활성화된다. 지구의 상세 지표는 현재 게임 생성 지형이다. [변경 범위](../production/20-galaxy-start-and-lobby.md).
