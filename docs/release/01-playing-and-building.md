...
시작 화면의 **우주 탐험 · 사업 시작**에서 혼자 또는 최대 6명으로 방을 운영한다. 목적지를 선택해 실제 항해·착륙한 뒤 `B`로 사업 단말을 연다. 무료 개발 등록 → 광맥 `F` 채광 → 현장 창고 반납 → 기술/시설 → 로봇 제작·배정 → 지역 환경 안정화 → 계약 정산 순서다. 건설은 실제 3D 미리보기를 조준해 클릭하며 `Esc`로 취소한다. 사업 자원은 출항 전에 반납하고 필요한 로봇은 정산 전에 회수한다. 공동 자금·기술·격납고와 지역 사업은 호스트의 `crew_world.json`에 보존된다. 개인 원본 장비와 기존 `campaign.json`/단독 탐험 기록은 별도다. 상세 조건과 검증은 [현장 사업](../production/13-expedition-industry-and-contracts.md)을 따른다.
