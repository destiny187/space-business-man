...
2026-09-14 **유실 화물 후속 정정**: T 식별 후 정지 상태에서 F 게이지를 채우는 동안 포드는 현 위치에 머물며 회수 거치대·선체 쪽 이동·견인선을 미리 생성하지 않는다. 호스트가 연결 완료를 저장한 뒤 견인선으로 연결하며, 그때부터 F를 놓아도 선체를 따라간다. 목적지 항만 인계에는 다시 F를 유지한다. 공전 기준 이동은 유지하며 물리 로프 시뮬레이션은 아니다. mine 정비 카트리지의 기존 적재 방식과 구분한다. [재현·수정·확인 기록](../production/192-freight-hold-and-home-version.md).
...
2026-09-10 SP05 구현: 새 기업 운항 규칙을 저장한 세계에서 태양계 CARRIER 2척을 지구–화성 왕복으로 연결했다. 설정·검수·기존 저장 경계는 [제작 기록](../production/113-space-y-freighters.md)에 둔다. 다른 항성계의 시드 항로는 SP07 후속이다.

2026-09-10 SP06 구현: 새 운항 설정의 지구/화성 항만에 WARDEN 2기씩을 연결했다. 근접 선박 위치 확인 후 복귀하며 플레이어의 항해 허가·피해·적대 관계를 바꾸지 않는다. [실제 범위](../production/114-space-y-patrol.md).

2026-09-10 후속 확정: 회사별 거점의 용도와 특색이 건물 전체 구조에서 드러나야 하며, 같은 거점의 심볼/상부 장비만 교체하는 방식으로 완료하지 않는다. SP07에서 새 시드의 여섯 진출권·관리 행성·기업 거점·행성 간 운항을 연결했다. Lotus 개척 보급·mine 집하/정비·CooperTech 전투로봇 시험/봉인 시설의 실제 모델과 동작 경계는 [소관 제작 기록](../production/115-seeded-corporate-regions.md)에 둔다. SP08의 상세 조사 흔적·후속 정비/사건과 구분한다.

## 2026-09-10 — SP08 활동 흔적 구현

새 세계의 실제 Lotus·mine·CooperTech 거점에 서로 다른 조사 실물을 배치한다. E 유지로 회사 식별 후 가까이에서 활동 기록을 확보하며, 호스트 저장 결과를 지도와 J 기업 도감에 공유한다. 기존 은하는 보존한다. 세 실물·공개 단계·수치·검수·후속 경계의 소관은 [SP08 제작 기록](../production/117-corporate-activity-traces.md)이다.

## 2026-09-10 — SP09 유실 화물 구현

새 세계의 태양계와 일부 운항 중인 시드 항로에 유실 포드를 배치한다. E 유지로 송장을 식별하고 가까이에서 윈치로 선박 외부 거치대에 회수해 지정 Space Y 항만까지 직접 운반한다. 인계와 공동 대금은 같은 저장으로 한 번 확정한다. 선박별 거치대·상대 정지·착륙 전 인계·수치·저장/렌더 확인의 소관은 [SP09 제작 기록](../production/118-freight-salvage.md)이다. 기존 은하와 플레이어 직접 운송 방향은 유지한다.

## 2026-09-10 — SP10 mine 정비 구현

산업 진출권의 정지 작업장에 교체 카트리지를 직접 운반하고 설치·정비해 재가동한다. 외부 거치대는 유실 화물과 공유하며 설치와 재가동을 별도 저장 단계로 구분한다. [소관 구현·수치·기존 저장·확인](../production/119-mine-maintenance.md).

## 2026-09-10 — SP11 CooperTech 지상 단서 구현

봉인 기록 정밀 조사에서 실제 시드 지상 폐기 로봇의 좌표를 확보하고 같은 사건을 발견·제압·회수한다. 출처와 종결 기록은 같은 지상 사건 ID를 사용한다. J·현장지도·방향 표식·기존 저장 경계는 [SP11 구현 기록](../production/120-coopertech-ground-clues.md)을 따른다.
