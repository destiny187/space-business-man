...
2026-09-15 새 원정 요구: 첫 출격 목적지는 철, 얼음, 구리를 실제 채집할 수 있고 물과 대기 적합도 최소 30을 보장하는 낮은 난이도 행성으로 고정한다. 규칙과 주소를 은하 설정에 보존한다. 지구 출발과 기존 저장의 생성 결과는 유지한다. [초기 구현값과 실제 확인](../production/197-early-play-fixes.md).

2026-09-10 최신 확정: 새 은하는 50만 행성으로 줄이되 은하 반지름을 조정해 현재와 비슷한 항성계 간격을 유지한다. 기존 100만 행성 저장은 보존한다. 태양계에서 은하 중심으로 향하는 좌측 탐험에 생명권을 집중하며, 생물의 자연 원산지는 같은 항성계도 예외 없이 한 행성뿐이다. [생명권 확장 진행 상태](../production/124-biota-8000-work.md).


2026-09-13 첫 화면 재정정: 새 원정은 **지구를 가까이 바라보며 Sol 표시 → 선체와 카메라가 자연스럽게 방향 전환 → 지구를 등지고 짧게 전진 → 업무 안내·직접 조작**으로 시작한다. 실제 위치는 인계까지 지구 근처에 유지하며 넓은 태양계 전경을 위해 멀어지지 않는다. 9월 10일의 전경 선회와 9월 12일의 지구를 계속 바라보는 후진은 과거 구현 이력이다. 새 경로·기존 저장 경계와 [실제 확인](../production/173-earth-departure-and-guide.md)을 따른다.

2026-09-10 사용자 확정: T1·T2 기초선 진행, T3~T5 선박 성능 조건을 적용한다. 기존 행성/항성권 분포는 유지하며, 항성권 진입과 목적 행성 착륙을 구분해 검사한다. 기존 조기 진출 제안보다 [우주선 진입 규칙](07-space-exploration-and-ships.md)의 최신 결정이 우선한다.

2026-09-10 [기업과 살아 있는 우주 공간](23-corporations-and-space-presence.md): 모든 새 시드의 복원 화성을 공통 기준점으로 두고 다른 기업 거점·활동·항로는 독립 시드 층으로 생성하는 초안이다. 화성의 Space Y 관리/테라포밍 완료는 사용자 방향이며 구체적 배치·밀도·저장 이행은 제안이다. 천체 주소·지질/동굴 시드를 바꾸거나 새 생성 코드를 구현한 기록이 아니다.
...
2026-09-10 최종 수정으로 새 은하의 행성 수는 **50만 개**다. 앞선 100만 개 기준은 기존 저장에 유지한다. 시드와 고정 주소로 전체 후보에 접근하고 필요한 천체만 생성·렌더링하며 변경분만 저장한다. 주 경험은 **3D 우주선 이동과 천체 탐험**이며 성도·목록은 보조 도구다. 목적지 선택만으로 항해를 모두 대신하지 않는다. [최신 제작·확인 범위](../production/124-biota-8000-work.md).

## 세계 생성과 항성계 — 2026-09-06 확정 방향

- 새 게임은 무작위 은하 시드 하나를 만들고 같은 세계를 저장·재개한다. 모든 새 세계의 출발지는 **태양계 지구**다.
- 50만 행성은 게임의 논리 주소 규모다. 실제 은하 행성 수의 관측 확정값이나 거리·반지름의 일괄 축척을 뜻하지 않는다. 은하 면적은 이전 대비 절반으로 줄여 항성계당 평균 면적을 유지한다.
...
초기 `galaxy-v3`는 125,000 항성계 × 8행성 = 1,000,000 주소를 지연 생성했다. 후속으로 항성계당 4~12개 가변 구성을 연결했으며, 2026-09-10 새 설정은 62,500 항성계·총 500,000 주소를 사용한다. 태양계 8행성의 이름·유형·순서와 지구 시작은 유지한다. 나머지 항성계는 독립 시드로 유형·궤도 위상을 정한다. 지구 도시·우주항 지표는 별도 제작 범위다.
...
이 단계에서 4~12개 행성의 가변 항성계와 소행성대·위성·고리·진입 구도를 연결했다. 당시 총 100만 행성과 태양계 8개를 유지했고, 최신 새 은하의 총수는 위의 50만 기준으로 변경했다. [당시 구성·주소·기존 저장 호환](../production/32-system-diversity.md).
