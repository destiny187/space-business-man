...
2026-09-10 첫 화면 변경: 새 원정은 **지구를 바라보며 Sol 표시 → 선체와 카메라가 함께 후진 → 여러 행성이 보이는 방향으로 선회 → 업무 안내·직접 조작**으로 시작한다. 사용자의 최종 정정은 태양이 아니라 지구를 첫 시선 대상으로 한다. 태양계 지구 출발 주소는 유지하되 실제 조작 인계 위치는 태양계 전경을 볼 수 있는 항로로 바뀐다. 아래 과거의 지구 근접 즉시 조작·첫 항성계 제목 생략보다 이 요구가 우선한다. [연출·저장·실제 확인](../production/20-galaxy-start-and-lobby.md).
