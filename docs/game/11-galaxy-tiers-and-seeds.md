...
2026-09-06 추가 요청의 소관 문서다. **하나의 은하를 기본 활동 범위로 사용하고, 행성을 티어·시드별로 생성하며, 우측 또는 은하 중심으로 갈수록 높은 티어가 나오는 구조**는 확정 요구사항이다. 생명체는 **준비된 사례별 모델·행동을 렌더링**하는 사용자 방향을 따른다. 아래 5티어·분포·지도·생물 사례는 구체화를 위한 설계 제안이며 미구현이다.
