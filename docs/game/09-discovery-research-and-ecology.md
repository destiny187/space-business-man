...
2026-09-14 발견 활용 첫 구현: 이슬 돌그릇과 열수 굴뚝의 조사 완료 기록으로 두 환경 설비를 연구하고 건설한다. 기존 기록과 원산 장소를 보존한다. [발견 소관](30-special-discovery-uses.md), [194 실제 범위](../production/194-discovery-industry.md)를 따른다.

2026-09-14 특수 발견 고도화는 [놀이, 장식과 실용 활용 소관](30-special-discovery-uses.md)에 상세화했다. 실제 발견 및 사건 목록, 특수 도감과 연구 후 제작, 일반 생태 전시와 미구현 후보를 구분한다. [개발 순서](../planning/20-special-discovery-and-facility-growth.md)를 따르며 이번 스캔 결과 표시 보완을 새 체험과 장식 콘텐츠 구현 완료로 해석하지 않는다.

2026-09-14 후속 반영: 단순 자연 발견의 T 기록 완료·즉시 광물/장비 보상 제거는 [188 분할 구현](../production/188-playtest-feedback-delivery.md)을 따른다. 아래 과거 대기 기록보다 해당 구현/검수 경계가 최신이다.

2026-09-14 **실플레이 수정 요구·구현 대기**: 단순 광물 단서 알림 제거, 공통 T 스캔/대상 연결 결과, 단순 발견의 재료 보상 제거·도감 유지는 7·9·11·14번을 따른다. 기존 정상 지급을 소급 회수하지 않는다. [18항목 요구·답변·코드 열람·분할 계획](../planning/19-playtest-feedback-september14.md)에 이번 변경의 단일 명세와 확인 경계를 둔다.

2026-09-12 묶음 확장: [159](../production/159-creature-remodel-batch.md)에서 기존 동물 56종의 표시를 추가 교체해 일반 원정 적용이 62종이 됐다. 종 ID·기본종 수·자연 원산지·기존 성향/전투 수치는 보존한다. 새 공격 기관 클립을 기존 비공격 종의 공격 활성화로 해석하지 않는다.

2026-09-12 전체 작업 후속: [157](../production/157-creature-remodel-field.md)의 6종 일반 원정 표시 교체는 종 ID·원산지·카탈로그 해시·전투 수치를 유지한다. 임포트/지형 보행을 확인한 8종과 실제 적용 6종을 구분하며, 호스트 돌진/도약·비행·저장된 생물 사건은 기존 표시를 유지한다.

2026-09-12 전체 리모델링 승인: 사용자가 대표 외형·동작·빠른 이동과 원형 타격 제거를 승인하고 전체 동물 제작 착수를 요청했다. 창작 방향을 위임했으며 신체 구조별 전용 리깅·공격 기관의 실제 위치를 따른다. 동물 5,600종의 제작 목록과 첫 추가 구조·기존 원산지/저장 보존은 [155](../production/155-creature-remodel-rollout.md)에 둔다.


2026-09-12 후속 승인: [153의 여섯 대표](../production/153-creature-type-studies.md)의 외형 품질을 유지하면서 이동·공격·특히 타격을 고도화한다. **여섯은 검토 대표 수이며 전체 생물의 리깅을 여섯 종류로 한정하지 않는다.** 사지·관절 축·몸통 분절·대칭/분기·날개/유영·공격 기관이 다르면 구조에 맞는 전용 리깅과 동작을 준비한다. [154의 적용과 분류·남은 범위](../production/154-creature-motion-and-contact.md)를 따른다.

2026-09-12 사용자 요청: 움직이는 생물의 외형과 동작을 기존 자산부터 확인한 [152 품질 조사](../production/152-creature-quality-audit.md)에 이어, **조형물 같은 특이함을 유지하며 생물다운 구조·이동·근접/원거리 공격을 함께 고려한 유형별 대표를 제작·재렌더**한다. [153의 독립 대표 6개](../production/153-creature-type-studies.md)는 시안이며 전종 교체·종수/원산지·저장 변경이나 실제 원정 전투 적용과 구분한다.

## 2026-09-12 — 발견 번호와 직접 명명

사용자 확정: 기존 몬스터 이름은 제작용 내부 구분 이름으로 남긴다. 플레이에서는 첫 발견을 종류 약어와 순번으로 기록하고 도감에서 직접 이름을 바꿀 수 있다.

- 현재 약어는 동물 `ANI`, 식물 `PLT`, 미생물 `MIC`다. 해당 원정에서 종류별 첫 스캔 성공부터 `001`, `002` 순으로 부여하며 1,000번째부터 자릿수를 늘린다.
- 번호는 기본종 단위다. 개체·색/크기 변이·재스캔·다른 행성으로의 표본 운송은 새 번호를 만들지 않는다. 종 ID·원산지·모델은 유지한다.
- 원정대의 공동 도감에서 이름을 최대 32자로 바꿀 수 있다. 번호는 바뀌지 않으며 이름과 번호 모두 검색할 수 있다. 이름을 지우면 번호 표시로 돌아간다.
- 변경은 호스트의 실제 저장 성공 후 확정되고 같은 원정의 승무원에게 공유된다. 다른 원정의 명명 기록과 구분한다.
- 일반 조준에는 이름/번호를 상시 표시하지 않는다. 스캔 결과·도감·표본/연구 선택 화면에서 사용하며 기존 스캔 결과 4초 규칙을 유지한다.
- 기존 저장에는 발견 시각이 없으므로 저장된 관측 기록 순서로 처음 번호를 부여한다. 이후에는 그 번호를 보존하며 과거의 실제 발견 시간 순서를 복원했다고 표현하지 않는다.

구현과 실제 확인 범위는 [150 제작 기록](../production/150-discovery-names.md)을 따른다.
