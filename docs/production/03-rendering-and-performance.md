...
2026-09-14 시야거리 질문 조사: 시야거리 설정의 원거리 지형과 광물 개체 생성 범위는 서로 다르다. 광물은 행성에 저장된 mineral_profile의 타일 크기와 view_radius로 주변을 스트리밍하고 모델 로딩을 분산한다. 현재 기본값은 80m 타일에 반경 1칸으로 3×3 타일이다. 최대 원거리 지형 10km가 광물도 10km에서 생성된다는 의미는 아니다. GPU의 메시 LOD 또한 별도다. 이번에는 차이를 확인했으며 광물 스트리밍 거리와 성능 예산 변경은 하지 않았다. [193 기록](193-field-and-navigation-followup.md).

2026-09-14 후속 반영: 강우 구름 반영·1.2~10 km 시야 실측과 최초 비용/회전 미재현 경계는 [188 분할 구현](188-playtest-feedback-delivery.md)을 따른다. 아래 과거 대기 기록보다 해당 구현/검수 경계가 최신이다.

2026-09-14 **실플레이 수정 요구·구현 대기**: 가장 먼 설정의 회전 복귀 지연과 실가시거리 조사, 강우/구름 표현은 8·13번을 따른다. 설정 10,000과 실제 식별 거리, 근접 상세와 원경을 구분하고 실측 전 프리셋 수치를 확정하지 않는다. [18항목 요구·답변·코드 열람·분할 계획](../planning/19-playtest-feedback-september14.md)에 이번 변경의 단일 명세와 확인 경계를 둔다.

2026-09-12 후속: 일반 거래의 동기 파일 대기와 날씨·사건의 반복 전체 초안을 줄였다. [160 측정·실제 창 확인](160-transaction-drafts.md)의 장비 선택 시간·지역 초안 비용을 따르며, 전체 검증·저장 스냅샷·산업 초안은 별도 잔여 비용이다.

2026-09-12 [단일 행동 범위 조사](158-action-scope-audit.md): 작은 행동의 전체 복사·저장·전송·화면 재생성을 필수 검사한다. 아이템창/배치/실패 요청의 수정과 일반 거래·정기 후보·지역 전송의 잔여 비용을 기록했다. 국소 함수 시간 개선을 전체 FPS 보장으로 해석하지 않는다.

## 2026-09-12 — 시야거리 다섯 단계와 채광 지연

이전 최대 4,800을 보통으로 두고 1,200 / 2,400 / 4,800 / 7,200 / 10,000 다섯 단계를 제공한다. 먼 두 단계는 바깥 지형 샘플을 완화해 생성량을 제한하며 가까운 세부 지형은 유지한다. 기존 사용자의 저장된 수치를 강제로 변경하지 않는다. 별 필터와 채광 메인 스레드 지연의 [수정·측정 조건·남은 한계](156-start-and-field-polish.md)를 따른다.

2026-09-12 지상 후속: [프레임 지연 개선](147-ground-frame-budget.md)에 생물 접지 예산·자동 저장 순서·지표/사업 화면 재사용·지도 작업 스레드와 실제 측정 범위를 기록했다. 안정적인 60 FPS 달성과 구분한다.

2026-09-12 현재 원정의 해적 교전: [전투 성능 조사](146-pirate-performance.md)에 불변 행성 정의·충돌 목록·효과 자원 재사용과 주기 저장 개선, M2의 보통 1280/1080 측정 및 미해결 순간 지연을 기록했다. 오프라인 전투 영상의 60 FPS와 구분한다.

2026-09-10 현재 원정: 모든 행성의 근접 표면을 개선하고 개인 행성 표면 품질을 3단계로 나눴다. 기본 맵·거리별 세부 샘플 비용·설정 이행·실제 확인의 소관은 [구현 기록](113-orbital-surface-quality.md)이다.

2026-09-09 현재 수면: 개인 수면 품질 낮음/보통/최상을 추가했다. 카툰 명암을 유지하며 바다의 정점 물결·잔물결과 하천/물리 물의 표면 표현을 구분한다. 기존 흰 점 반복을 제거하고 거리별 세부 감소·메시 재사용을 적용했다. [현재 원정의 수면 옵션·비교·확인 한계](88-water-quality-options.md).

2026-09-09 현재 필드: 개인 거리 설정을 유지하면서 지형·시설 오클루전과 화면 밖 생물/시설의 모션 갱신 생략을 적용했다. 호스트의 물리·권한 계산은 계속한다. [적용·실제 확인·멀티플레이 경계](86-field-visibility-optimization.md).

2026-09-09 후속: 외부 비행 영상에 가려진 메인 3D는 렌더를 중단하며 선내 복귀/착륙 전환에서 복구한다. 공통 정적 모델 미리보기는 변경 때만 다시 그리고, 연구·신체 증강의 움직이는 미리보기는 연속 렌더를 유지한다. [실제 적용·확인·제약](84-covered-world-and-preview-rendering.md).
