...
1200px 출력 높이 기준 외곽 탐색 반경은 3px, 내부는 0.8px다. 식생 단독 촬영은 2.5px/0.55px를 사용한다. 이는 최종 선 전체 폭이 아니라 경계 검출 반경이다. 해상도·렌더 스케일에 따라 보정하므로 고해상도에서 선이 상대적으로 가늘어지지 않는다. 게임 내부 선은 카메라 깊이 15~55m에서 점차 억제한다. 외곽은 남기되 깊이 28~110m에서 대비를 24%까지 낮춘다. 연속된 지면의 원근 기울기는 깊이 불연속 검출에서 상쇄한다. 깊이 변화 0.007m 미만의 작은 양각 내부 선은 억제한다. 주변 깊이 샘플의 표면 연속성이 낮은 가는 부속물은 선 강도를 45%까지 줄인다. 큰 외곽 탐색 반경은 유지한다. 2026-09-08 사용자 승인에 따른 내부·원거리 선 조정이며 [행성·생물 선별 개선 기록](54-planets-and-life-art.md)에 전후 렌더를 보존한다.
...
후속 저장 정책은 [에셋 버전 관리](../technical/05-asset-version-control.md)를 따른다. 대표 참조·기본 형상 렌더·검토 모음판은 LFS로 보존하고 생물 전수 변형·조명·LOD 개별 캡처는 필요할 때 로컬 재생성한다. 원본/게임 자산과 필수 시각 품질 기준은 유지한다.

## 태양계 정점 채색 연결

[Blender 태양계 제작](21-solar-system-blender.md)에서 `_vertex_paint` 재질 이름을 공통 정점색 변환에 연결했다. GLTF의 COLOR_0는 유지되지만 가져온 재질의 플래그가 꺼져 있는 경우도 같은 색을 사용한다. 공통 조명의 `highlight_strength` 기본값은 1이며 자연 천체에서만 과도한 반사를 낮춘다. 3단 명암과 공통 윤곽은 유지한다.

## UI의 정보 표현

UI의 글자·여백·상태·시각 표현 우선 원칙은 [FIELD v1](22-uiux-guide.md)을 따른다. 월드/미리보기 모델은 INK v1을 유지하며, UI 본문에 월드용 굵은 윤곽선을 강제하지 않는다.

## Blender 제작 설정 공통화 — 2026-09-08

[공통 제작 규격과 그래픽 통일](49-common-blender-art.md)에 기존 자산 비교, 산업 재질 역할 데이터, Blender 공통 함수, SCOUT/창고 적용과 남은 형태 교체를 기록한다. 선 굵기·3단 명암은 이 문서의 기존 기준을 유지한다. 재질 호환 적용과 모델 재제작 완료를 구분한다.
