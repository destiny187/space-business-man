...
func run() -> void:
	folder=ProjectSettings.globalize_path("res://../docs/production/media/planet-coverage/solar")
	...
	for i in 8:
		...
		solar.set_process(false);solar.detail.hide();solar.distant.show()
		await capture(FrontierSolarPlanet.ASSETS[i]+"-far")
	...
	var result={"checks":checks,"failed":failed,"renderer":RenderingServer.get_current_rendering_method(),"device":RenderingServer.get_video_adapter_name(),"scope":"8 production SolarPlanet previews, explicit near/far, no navigation fixture or resource rules changed"}
