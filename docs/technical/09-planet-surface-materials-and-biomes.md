...
2026-09-13 사용자 승인: 반복감을 줄이기 위해 재질 18종을 추가해 43종으로 확장하고, 행성별 주/보조 퇴적물·무늬 방향/좌표 변형·넓은 풍화색을 실제 지표에 연결한다. 기존 저장의 물리 지형·지역 판정을 보존하며 상세 제작과 확인은 [170 제작 기록](../production/170-surface-material-expansion.md)을 따른다. 아래 과거 첫 라이브러리 수량은 설계 이력이다.

2026-09-11 사용자 승인: 자연 퇴적 재질과 복원용 지면을 추가하고 실제 환경값에 따른 단계 변화·지역 경계를 개선한다. [테라포밍 지면 구현·검수](../production/134-terraform-surface-progression.md)를 따른다.

2026-09-11 사용자 승인: 재질 변형·계열별 지형·행성 내 지역 조합을 단계별로 개발한다. 기존 저장 지형을 보존하는 적용 경계와 완료/검수 상태는 [행성 다양성 작업](../production/133-planet-variety.md)을 따른다.

2026-09-11 사용자 요청 후속: 자유 복원 환경 텍스처는 실제 복원 칸 경계로 줄여 재사용하고, 화산 지표는 Blender의 `lava_crust` 패킹 텍스처로 현무암 판·균열·국소 고온부를 표현한다. 같은 10종·1024 텍스처 배열에서 교체하며 지형/자원 생성과 저장 수치는 유지한다. [실제 변경·렌더·성능·제약](../production/132-lava-and-surface-efficiency.md)을 따른다.

2026-09-10 궤도 표면 후속: 생성 18종·태양계·복원 화성의 연속 필드 맵, 가까운 지질 무늬, 픽셀 경계 보정과 개인 품질 3단계는 [근접 표면 구현](../production/113-orbital-surface-quality.md)을 따른다. 지상 재질과 궤도의 실제 지형 일치를 추가한 것은 아니다.

2026-09-10 [지역 복원 우주 외형](../production/112-orbital-terraforming.md): 저장된 환경을 궤도 근/원거리와 항법 미리보기에 연결했다. 지표는 최대 16개 공간 묶음과 식별용 최소 반경, 대기는 기존 구면 원장을 사용한다. 아래의 정확한 구면 해안선·착륙 지리 통합은 여전히 후속이다.

2026-09-09 후속 [지역 테라포밍·지표 수급 설계](../game/20-regional-terraforming-and-surface-supply.md): 같은 기반 지질/지형 샘플로 지역 경계·지표 광물 군집·Tab 지도를 연결하고 가변 복원 피복을 광물 생성에서 분리한다. 지하 시드·기존 세계는 보존한다. 아래 공통 표면 샘플·전 행성 지도 구상을 실제 구현 완료로 바꾸는 기록이 아니며, 새 지역 생성과 계약의 범위/버전 제안은 새 문서를 따른다.
